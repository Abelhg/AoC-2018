# Advent Of Code: Dia 24 - Segunda parte
...
def generaGrupo(ID, cadena, ejercito, bonus=0):
	vals = cadena.split()
	num = int(vals[0])
	vid = int(vals[4])
	ata = int(vals[-6]) + bonus
	tip = vals[-5]
	ini = int(vals[-1])
	# Inmunidades
	inmus = []
...
cont = 1
for i in inmu:
	GRUPOS.append(generaGrupo("INM-"+str(cont),i, "INM", BONUS))
	cont += 1
...
while True:
	...
	if len(gINM) == 0:
		print "Gana la infeccion", BONUS
		break
	if len(gINF) == 0:
		print "Gana el sistema inmunitario", BONUS
		break
	...
	while i < len(GRUPOS):
		grupo = GRUPOS[i]
		#print grupo.objetivo
		if grupo.objetivo != None:
			d = calcularDanio(grupo, grupo.objetivo)
			# Ataca
			grupo.objetivo.nUnidades -= int(float(d) / grupo.objetivo.vida)
			#print grupo.ID, "ataca a", grupo.objetivo.ID, ". Le quita", int(float(d) / grupo.objetivo.vida), "y le deja con", grupo.objetivo.nUnidades,"unidades"
...
print "Solucion:", sum(g.nUnidades for g in GRUPOS), "con bonus", BONUS